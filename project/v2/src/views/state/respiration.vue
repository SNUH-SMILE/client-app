        ...
        <div class="chart-data-wrap" v-if="detail.length > 0">
          <div class="chart-box">
            <div class="chart-mark">
              <span class="chart-mark-circle">호흡수</span>
            </div>
...
import { HEART_DETAIL, RESPIRATION_DETAIL } from '@/modules/health';
...
export default {
  ...
  computed: {
    ...mapGetters({ detail: RESPIRATION_DETAIL }),
    labels() {
      return this.detail.map(({ timeLabel }) => timeLabel);
    },
    datas() {
      return this.detail.map(({ rr }) => {
        return Number(rr);
      });
    },
  },
  methods: {
    ...mapActions({ fetchDetail: RESPIRATION_DETAIL }),
