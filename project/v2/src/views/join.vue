            ...
            <div class="form-box tb-half">
              ...
              <validation-provider name="성명" rules="required" slim>
                <text-field id="name" class="tb-inblock" label="성명" disabled v-model="state.patientNm">
                  <template>
                    <button type="button" class="btn-line-rnd" @click="$alert('작업 대기중')">본인 인증</button>
                  </template>
                </text-field>
              </validation-provider>
...
export default {
  ...
  methods: {
    ...
    async regist() {
      if (this.dupLoginId !== this.state.loginId) return this.$alert('아이디 중복체크를 진행해주세요.');
      // TODO: 본인인증 처리
      // if(this.authCheck)return this.$alert('본인인증을 진행해주세요.');
