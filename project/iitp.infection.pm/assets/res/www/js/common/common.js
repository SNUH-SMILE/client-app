...
var openPopup = function(page, param, el, func){
    ...
    $.get("../../html/" + page.substring(0,3) + "/" + page + ".html", function(data){
	...
	//		Global.popupPage = page;
	    ...
	    temp.attr("layer-"+page, ""); //layer ID 부여
	    	...
	    	if (temp.outerHeight()*2 < $(document).height() )//팝업이 높이가 짧은 경우
	    		temp.css('margin-top', '-'+temp.outerHeight() / 1.3+'px');
            else if(temp.outerHeight() < $(document).height() )//팝업 높이가 긴 경우
...
function makeDataArr(type,rows){
    ...
    var data = [];
    //혈압에서만 사용
    var setData = [], lowData = [], highData = [];
    //수면에서만 사용
    var diffSum = 0, diffSum0 = 0, diffSum1 = 0, diffSum2 = 0;
    var option = {};
    ...
    for(var i = 0; i < rows.length; i++){
        ...
        if(type == 'step'){
            ...
            if(moment(row.sleepStartDate+row.sleepStartTime,"YYYYMMDDHHmmss", true).isValid() && moment(row.sleepEndDate+row.sleepEndTime,"YYYYMMDDHHmmss", true).isValid()){
                var t1 = moment(row.sleepStartDate+row.sleepStartTime,"YYYYMMDDHHmmss");
...
function onReceiveAndroidNotification(ns){
    ...
    var pushArr = MAPI.data.storage("g","PUSH_RCV_LIST") ? MAPI.data.storage("g","PUSH_RCV_LIST") : [];
    if(pushArr.length == 0 || (pushArr.length > 0 && data.msgSeq != pushArr[pushArr.length-1].msgSeq)){
        pushArr.push(data);
...
function getPushData(ns){
    ...
    var imageURL="", remoteURL, messageUID, title = "알림", message = "", extData = "", category = "";
    var pushInfo = M.plugin("push").info();

    // APNS: {"mps":{"senddate":"2017021409","ext":"3%7C184%2Ctext%7Chttp%3A%2F%2F211.241.199.216%3A28080%2Fsecure-push-gw%2Ficon-default.png","sender":"u19","seqno":"1254","db_in":"N","pushkey":"1074e6143e28190a1c9518bf735052eac6b74b2f"},"aps":{"badge":5,"alert":"%E3%85%87%E3%84%B9%E3%85%87%E3%84%B9%E3%85%8E%E3%85%8E%E3%85%8E%E3%85%8E","sound":"alert.aif"}}
